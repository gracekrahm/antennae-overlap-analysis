...
def parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        help="FITS file where the found clumps will be saved.  If OFITS "
             ...
             "(default: ifits with modified extension label)"
        )
    parser.add_argument(
        "--otext",
...
def set_defaults(options, idata):
    ...
    # ofits -- ifits with modified extension label
    ...
    if (new_options.dTleaf is None) or (new_options.Tcutoff is None) or (new_options.Tpkmin is None):
        ...
        noise.mask = (~np.isfinite(idata)) | (idata > nsig*std_data)
        # NOTE: Numpy's std() takes memory of ~6 times idata size.
        std_noise = noise.std()
        ...
        if new_options.Tpkmin is None:
            LOGGER.debug("Setting Tpkmin to %f (= pkmin*std_noise = pkmin*%f)",
                         pkmin*std_noise, std_noise)
            new_options.Tpkmin = pkmin*std_noise
